...
exports.getTopPlayerOfCategory = (req, res) => {
  ...
  User.findOne(
    { ['stats.category.' + category]: { $exists: true } },
    (err, user) => {
      if (user) {
        User.find({})
          .sort({ ['stats.' + category]: -1 })
          .exec(function (err, descendingOrder) {
            res.status(200).json({ descendingOrder });
          });
      } else {
        res
          .status(400)
          .json({ message: "Category doesn't exist or never played" });
      }
    }
  );
};
    ...
    User.updateOne(
      { _id: new mongodb.ObjectID(decoded.userId) },
      {
        $inc: {
          ...
          ['stats.category.' + categoryName + '.nbQuizzWon']: req.body
            .nbQuizzWon,
          ['stats.category.' + categoryName + '.nbQuizzLost']: req.body
            .nbQuizzLost,
        },
        // '$set': {
